...
인식이 되면
내가 label_persent 파일에 정의한 값과 비교하여 개별 퍼센트로 한번 걸러내고
개별퍼센트로 걸러낸 것을 가지고 화면조건파일과 비교하여 현재화면을 얻어내는데
방법은 개별로 가지고 있는 인식대상과 화면이 1대n인지를 비교하여 1대1이면 바로 현재화면 반환하고
아니면 과거화면 기준으로 현재화면을 반환하는데 같은 화면의 여러번의 중복요청이 있을 경우가 있으니
각 내역을 만들어 1대n의 현재화면을 얻어낸다음 화면내역의 마지막 화면과 일치하고 30초내의 요청일 경우
화면내역에는 넣지 않고 반환하여 네모이펙트는 여전히 출력시킨다.
